...
#import "CK2CalendarEvent.h"

@class CK2PagedResponse;

@interface CK2CalendarEvent (Networking)
 ...
 */
+ (void)fetchCalendarEventsFrom:(NSDate *)startDate to:(NSDate *)endDate success:(void(^)(CK2PagedResponse *events))success failure:(void(^)(NSError *error))failure;
