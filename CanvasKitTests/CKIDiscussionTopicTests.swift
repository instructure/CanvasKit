...
class CKIDiscussionTopicTests: XCTestCase {
    ...
    func testJSONModelConversion() {
        var discussionTopicDictionary = Helpers.loadJSONFixture("discussion_topic") as NSDictionary
        var topic = CKIDiscussionTopic(fromJSONDictionary: discussionTopicDictionary)
        
        var formatter = ISO8601DateFormatter()
        formatter.includeTime = true
        
        XCTAssertEqualObjects(topic.title, "Topic 1", "Discussion Topic title was not parsed correctly")
        
        XCTAssertEqualObjects(topic.messageHTML, "<p>content here</p>", "Discussion Topic messageHTML was not parsed correctly")
        
        var url = NSURL.URLWithString("https://canvas.instructure.com/courses/1/discussion_topics/2")
        XCTAssertEqualObjects(topic.htmlURL, url, "Discussion Topic htmlURL was not parsed correctly")
        
        var date = formatter.dateFromString("2037-07-21T13:29:31Z")
        XCTAssertEqualObjects(topic.postedAt, date, "Discussion Topic postedAt was not parsed correctly")
        
        date = formatter.dateFromString("2037-07-28T19:38:31Z")
        XCTAssertEqualObjects(topic.lastReplyAt, date, "Discussion Topic lastReplyAt was not parsed correctly")
        ...
        XCTAssertEqualObjects(topic.assignmentID, "847", "Discussion Topic assignmentID was not parsed correctly")
        
        date = formatter.dateFromString("2037-07-21T13:29:31Z")
        XCTAssertEqualObjects(topic.delayedPostAt, date, "Discussion Topic delayedPostAt was not parsed correctly")
        
        XCTAssertEqualObjects(topic.isPublished, true, "Discussion Topic isPublished was not parsed correctly")
        
        date = formatter.dateFromString("2037-07-21T13:29:31Z")
        XCTAssertEqualObjects(topic.lockAt, date, "Discussion Topic lockAt was not parsed correctly")
...
//        XCTAssertEqualObjects(topic.lockInfo.dynamicType, CKILockInfo().self, "Discussion Topic lockInfo.class was not parsed correctly")
        
        XCTAssertEqualObjects(topic.lockExplanation, "This discussion is locked until September 1 at 12:00am", "Discussion Topic lockExplanation was not parsed correctly")
        
        XCTAssertEqualObjects(topic.userName, "User Name", "Discussion Topic userName was not parsed correctly")
        
        XCTAssertEqual(topic.childrenTopicIDs.count, 3, "Discussion Topic childrenTopicIDs was not parsed correctly")
        
        XCTAssertEqualObjects(topic.rootTopicID, "1236", "Discussion Topic rootTopicID was not parsed correctly")
        
        url = NSURL.URLWithString("/feeds/topics/1/enrollment_1XAcepje4u228rt4mi7Z1oFbRpn3RAkTzuXIGOPe.rss")
        XCTAssertEqualObjects(topic.podcastURL, url, "Discussion Topic podcastURL was not parsed correctly")
