...
@interface CKTestClient : CKClient
@property (nonatomic) NSMutableSet *errorPaths;
@property (nonatomic) NSMutableDictionary *objectsByPath;
@end

@implementation CKClient (TestingClient)

+ (CKClient *)testClient
{
    static CKClient *testClient;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        testClient = [CKTestClient new];
    });
    return testClient;
}
...
- (AFHTTPRequestOperation *)GET:(NSString *)URLString parameters:(NSDictionary *)parameters success:(void (^)(AFHTTPRequestOperation *, id))success failure:(void (^)(AFHTTPRequestOperation *, NSError *))failure
{
    ...
    if ([self.errorPaths containsObject:URLString]) {
        failure(nil, [NSError errorWithDomain:@"com.instructure.CanvasKit" code:0 userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"CKTestClient error.", @"Error only used in testing")}]);
        return nil;
    }
...
//    
//    if ([self.errorPaths containsObject:URLString]) {
//        failure([NSError errorWithDomain:@"com.instructure.CanvasKit" code:0 userInfo:@{NSLocalizedDescriptionKey: NSLocalizedString(@"CKTestClient error.", @"Error only used in testing")}]);
//        return nil;
//    }
//    
//    id responseObject = self.objectsByPath[URLString];
